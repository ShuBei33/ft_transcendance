...
export class ChannelService {
  ...
  async createChannel(
    ...
    try {
      ...
      if (hash) newChannel.hash = await bcrypt.hash(hash, 10);
  ...
  async updateChannel(
    ...
    try {
      ...
      if (
        !channelModified.name &&
        !channelModified.visibility &&
        !channelModified.hash
      )
