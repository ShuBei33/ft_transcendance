...
export class DiscussionService {
	...
	async create_discussion( userId_1: number, userId_2: number ) : Promise<Discussion>
	{
	...
	async create_discussion_message( userId: number, payload: DTO_SocketDiscMessage)
	{
	...
	async get_discussion( userId: number, discId: number ) : Promise<DiscussionLite>
	{
	...
	// WARNING 
	async get_discussion_full( userId: number, discId: number ) : Promise<Discussion>
	{
        const discussions = await this.prisma.discussion.findFirst({
            where: { id: discId,
				OR: [
	...
	async change_discussion_status( userId: number, payload: DTO_ChangeStatusDiscussion ) : Promise<void>
	{
