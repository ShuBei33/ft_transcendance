...
export class handle {
  ...
  async AddFriend(receiverId: number) {
    await this.Friend.sendInvitation(receiverId)
      .then(({ data }) => {
  ...
  async FriendshipRemove(friendId: number) {
    await this.Friend.deleteFriend(friendId)
      .then(({ data }) => {
        const friendshipRemove = data.data;
        const userIdToRemove =
          friendshipRemove.senderId == get(user)?.id
            ? friendshipRemove.receiverId
            : friendshipRemove.senderId;
        dataStore.update((prev) => {
  ...
  async FriendShipBlock(receiverId: number) {
    await this.Friend.blockUser(receiverId)
      .then(({ data }) => {})
      .catch((e) => {});
  }
  ...
  async FriendShipUnBlock(friendship: Friendship) {
    const receiverId =
      friendship.receiverId == get(user)?.id ? friendship.senderId : friendship.receiverId;
    await this.Friend.unBlockUser(receiverId)
      .then(({ data }) => {
